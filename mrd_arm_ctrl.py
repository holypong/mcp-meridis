"""
mrd_arm_ctrl.py - ROID1 両腕 IK ライブラリ
...
MJCF軸 (左腕):
  l_shoulder_pitch  : Y軸回り (sp)  負=前に振る, 正=後に振る
  l_arm_upper_roll  : X軸回り (sr)  正=外に上げる(左方向), 負=内側
                     ※左腕は物理角とMeridim角が同符号
  l_elbow_yaw       : Z軸回り (ey)  前腕の捻り
  l_arm_lower_pitch : Y軸回り (ep)  負=屈曲, 正=過伸展
...
@dataclass
class ArmAngles:
    """各関節角度 [deg] — Meridim コマンド値と同一符号 (左右共用)"""
    shoulder_p: float  # 肩ピッチ
    shoulder_r: float  # 肩ロール (正=外方向 — 左右とも同じ符号定義)
    elbow_y:    float  # 肘ヨー
    elbow_p:    float  # 肘ピッチ
...
def shoulder_pos_left(params: ArmParams) -> np.ndarray:
    """左肩関節位置 (waist frame) [m]"""
    return np.array([params.shoulder_x, params.shoulder_y, params.shoulder_z])
...
def compute_left_arm_ik(
    ...
    ロール符号:
      左腕は物理角とMeridim角が同符号 (右腕と異なり符号反転なし)
    """
    L1, L2 = params.L1, params.L2
    S = shoulder_pos_left(params)
    ...
    # 左腕ロールは符号反転なし (物理角 = Meridim角)
    def _score(sol: tuple[float, float, float]) -> tuple[int, float]:
        sp_d = math.degrees(sol[0])
        sr_d = math.degrees(sol[1])    # 物理角 = Meridim角 (符号反転なし)
        ep_d = math.degrees(sol[2])
        in_lim = (
            ARM_LIMITS["shoulder_p"][0] <= sp_d <= ARM_LIMITS["shoulder_p"][1] and
            ARM_LIMITS["shoulder_r"][0] <= sr_d <= ARM_LIMITS["shoulder_r"][1] and
    ...
    return clamp_arm_angles(ArmAngles(
        shoulder_p=math.degrees(sp),
        shoulder_r=math.degrees(sr),   # 物理角 = Meridim角: 符号反転なし
        elbow_y=elbow_y_deg,
        elbow_p=math.degrees(ep),
    ))


def compute_left_arm_fk(angles: ArmAngles, params: ArmParams) -> np.ndarray:
    """
    左腕 FK: 関節角度 → 手先位置 (waist frame)
    elbow_yaw は手先位置に影響しないため無視する
    """
    sp = math.radians(angles.shoulder_p)
    sr = math.radians(angles.shoulder_r)   # Meridim左腕ロールは符号反転なし
    ep = math.radians(angles.elbow_p)

    # 上腕方向 (waist frame): R_y(sp) · R_x(sr) · (0, 0, -1)
    u = np.array([
        -math.cos(sr) * math.sin(sp),
         math.sin(sr),
        -math.cos(sr) * math.cos(sp),
...
def right_arm_angles_to_meridim(angles: ArmAngles) -> dict[int, float]:
    """右腕 ArmAngles → Meridim インデックスの辞書"""
    return {
...
def left_arm_angles_to_meridim(angles: ArmAngles) -> dict[int, float]:
    """左腕 ArmAngles → Meridim インデックスの辞書"""
    return {
        22: angles.shoulder_p,
        24: angles.shoulder_r,
        26: angles.elbow_y,
        28: angles.elbow_p,
    }
...
if __name__ == "__main__":
    ...
    print(f"右肩位置 (waist frame): {shoulder_pos_right(params)}")
    print(f"左肩位置 (waist frame): {shoulder_pos_left(params)}")
    ...
    for label, target in right_targets:
    ...
    for label, target in left_targets:
        angles = compute_left_arm_ik(target, params)
        fk = compute_left_arm_fk(angles, params)
        err = float(np.linalg.norm(fk - target))
        print(f"[{label}]")
        print(f"  target: {target}")
        print(f"  IK  sp={angles.shoulder_p:7.2f}° sr={angles.shoulder_r:7.2f}°"
