...
私は**Meridian**プロジェクトを通じて、この課題解決に取り組んでいる。
...
> | [merimujoco](https://github.com/holypong/merimujoco) | holypong | MuJoCo 物理シミュレーション。Sim2Real / Real2Sim・デジタルツイン同期を meridis 経由で提供 |
> | [meridis](https://github.com/holypong/meridis) | holypong | Redis ベースのデータブリッジ。5 つの Redis キー（各 90 要素ハッシュ）でシミュレータ・制御・MCP を接続 |
> | **mcp-meridis（本プログラム）** | holypong | MCP サーバー。AI エージェントが歩行制御・パラメータ調整・データ収集を操作する |

**① 実機との直結：指令と応答データの双方向取得**
本プログラムが生成する歩行指令（関節角度列）は meridis の Redis キー（`meridis_mcp_pub`）を通じてそのまま実機で再生できる。同時に、実機の応答値・IMU センサ値を `meridis_sim_pub` 等から取得できる。シミュレーションデータでは得られない **実機の物理応答**を研究データとして直接扱える点は、現在のロボティクスAI 研究において希少な資産である。

**② 古典アルゴリズムから AI への一貫したパイプライン**
本仕様では IK ベースの周期歩容（ZMP 理論に裏付けられた解釈可能な歩行生成）を起点とし、その出力を **模倣学習（Behavior Cloning）の理想デモデータ** として Genesis AI 等の強化学習エージェントへ渡す設計とする。merimujoco を用いてシミュレーション上で歩容を検証し、Sim2Real で実機に転移する。実機歩容は Real2Sim で再びシミュレーションに取り込み、定量比較が可能となる。この「古典制御 ⇔ 学習制御 ⇔ シミュレーション」の多方向パイプラインが、理論的解釈可能性と学習の柔軟性を両立させる研究基盤となる。

**③ AI エージェントによる実験の自動化（MCP 連携）**
本プログラムは MCP（Model Context Protocol）サーバー機能を備え、Claude 等の AI エージェントが歩行指令・パラメータ設定・データ収集をプログラム的に操作できる。MCP 経由の歩行指令は meridis の `meridis_mcp_pub` キーに書き込まれ、merimujoco シミュレータまたは実機に即座に反映される。これにより、パラメータ探索・歩行実験・データ解析のループを AI が自律的に回す実験環境を構成できる。

### Sim-to-Real への接続

merimujoco は MuJoCo 物理シミュレーション上で Roid1（KHR-3HV 相当）モデルを動作させ、meridis の Redis キーを通じて Sim2Real（シミュレーション → 実機）および Real2Sim（実機 → シミュレーション）のデータ転送を実現している。本プログラムが `meridis_mcp_pub` に書き込んだ歩行指令は、merimujoco のデジタルツイン上でも同時に検証できる。さらに meridis は Genesis AI・Isaac Sim にも対応しており、シミュレータを切り替えて同一歩容を比較する拡張も可能である。
