...
フィジカルAI（Physical AI）とは、物理世界を理解し実体を持って行動する知能システムの総称であり、二足歩行ロボットはその最も挑戦的な対象の一つである。近年の研究（Genesis、Isaac Lab、π₀ 等）では、強化学習や模倣学習によって高品質な歩容を獲得する手法が急速に発展している。しかし、学習された歩容の **実機での再現性と安定性の検証** は依然として困難であり、良質なデモデータと実機検証環境の不足が研究の障壁となっている。

本プロジェクトはこの課題に対して、次の3点で具体的な価値を提供する。

**① 実機との直結：指令と応答データの双方向取得**
本プログラムが生成する歩行指令（関節角度列）は Meridian プロトコルを通じてそのまま実機で再生できる。同時に、実機から関節角度の応答値・加速度・ジャイロ・DMP 推定姿勢角（IMU センサ値）を同一プロトコルで収集できる。シミュレーションデータでは得られない **実機の物理応答**を研究データとして直接扱える点は、現在のロボティクスAI 研究において希少な資産である。

**② 古典アルゴリズムから AI への一貫したパイプライン**
本仕様では IK ベースの周期歩容（ZMP 理論に裏付けられた解釈可能な歩行生成）を起点とし、その出力を **模倣学習（Behavior Cloning）の理想デモデータ** として Genesis AI 等の強化学習エージェントへ渡す設計とする。逆に、強化学習で獲得した歩容を本システムで実機検証する。この「古典制御 ⇔ 学習制御」の双方向パイプラインは、理論的解釈可能性と学習の柔軟性を両立させる研究基盤となる。

**③ AI エージェントによる実験の自動化（MCP 連携）**
本プログラムは MCP（Model Context Protocol）サーバー機能を備え、Claude 等の AI エージェントが歩行指令・パラメータ設定・データ収集をプログラム的に操作できる。これにより、パラメータ探索・歩行実験・データ解析のループを AI が自律的に回す実験環境を構成できる。

### Sim-to-Real への接続

Meridian プロトコルは Genesis・Isaac Sim・ROS・MuJoCo との接続実績を持つ。「シミュレーションで学習 → 実機で検証」の Sim-to-Real パイプラインへの接続は、すでにある既存資産として活用できる。

### ZMP：歩容の検証指標として

ZMP（Zero Moment Point）は動的歩行の安定性を定量的に評価する理論的基盤である。本仕様では ZMP を **歩容の良否を判断する検証指標** として位置づける。強化学習やアルゴリズムが生成した歩容に対し、ZMP が支持多角形内に収まるかをオフライン解析することで、安定性を理論的に説明できるようにする。

### 読み進め方

実装の初期フェーズは **意図的な簡略化** を含んでおり、これらは不具合ではなく段階的に解消すべき設計上の近似とする。各フェーズの理論的背景を理解した上で実装・実験を進めることで、基礎理論から最新のフィジカルAI 研究への道筋を一貫して体験できる構成としている。
...
8. [フェーズ 5 — MCP によるパラメータ調整と歩行データ収集](#8-フェーズ-5--mcp-によるパラメータ調整と歩行データ収集)
9. [フェーズ 6 — 遊脚軌道改善と腕振り制御](#9-フェーズ-6--遊脚軌道改善と腕振り制御)
10. [フェーズ 7 — IMU センサフィードバック（ジャイロミキシング）](#10-フェーズ-7--imu-センサフィードバックジャイロミキシング)
11. [フェーズ 8 — ZMP 解析による定量的評価](#11-フェーズ-8--zmp-解析による定量的評価)
12. [フェーズ 9 — フィジカルAI 連携（模倣学習・Sim-to-Real）](#12-フェーズ-9--フィジカルai-連携模倣学習sim-to-real)
13. [フェーズ 10 — 発展的研究テーマ](#13-フェーズ-10--発展的研究テーマ)
14. [現行実装の評価と次のステップ](#14-現行実装の評価と次のステップ)
15. [対象ロボットの物理仕様](#15-対象ロボットの物理仕様)
16. [用語集](#16-用語集)
17. [参考文献](#17-参考文献)
...
| P0-4 | 対象ロボットの関節構成（§10）とリンク長を理解し、JSON ファイルからパラメータを読み込めること | レビュー |
...
## 8. フェーズ 5 — MCP によるパラメータ調整と歩行データ収集

### 8.1 目的

フェーズ 4 で安定した歩行状態遷移が実現した。次のステップとして、センサフィードバックを加える **前** に、現在のオープンループ歩行のパラメータを体系的に調整し、最良の基準歩容を確立する。MCP サーバー機能を活用して、AI エージェントがパラメータ変更・実験実行・データ収集を自動的に繰り返す研究サイクルを構築する。
...
## 9. フェーズ 6 — 遊脚軌道改善と腕振り制御

### 9.1 目的

フェーズ 5 で最良の基準パラメータを確立した後、オープンループ歩行そのものの品質を向上させる。遊脚の軌道形状を改良して着地衝撃を低減し、腕振りによる角運動量補償を加える。
...
## 10. フェーズ 7 — IMU センサフィードバック（ジャイロミキシング）

### 10.1 目的

フェーズ 5〜6 でオープンループ歩行の品質を高めた後、初めてセンサフィードバックを導入する。IMU（慣性計測ユニット）の角速度データを用いたジャイロミキシングにより、外乱に対してロバストなクローズドループ制御を実現する。

### 10.2 理論的背景

#### オープンループとクローズドループの違い

フェーズ 3〜6 の歩行は、計画した軌道をそのまま再生する **オープンループ制御** である。床面の凹凸・サーボ応答遅れ・リンク長誤差などにより姿勢がずれても修正されない。
...
## 11. フェーズ 8 — ZMP 解析による定量的評価

### 11.1 目的

フェーズ 5 以降で収集したデータを用いて、歩行安定性を ZMP 理論で定量的に検証する。目視確認だけでは判断できない安定性の根拠をデータで示す。
...
## 12. フェーズ 9 — フィジカルAI 連携（模倣学習・Sim-to-Real）

### 12.1 目的

フェーズ 5〜8 で整備した歩行基盤とデータ収集環境を活かし、フィジカルAI 研究へのパイプラインを構築する。本システムが生成する実機検証済みの歩容を模倣学習のデモデータとして活用し、強化学習エージェントとの双方向連携を実現する。
...
シミュレーションで学習した方策を実機に転移する際、シミュレーションと現実の物理特性の差異（摩擦・質量分布・アクチュエータ応答等）により性能が低下する現象。Meridian プロトコルは Genesis・Isaac Sim・ROS・MuJoCo との接続実績を持ち、このギャップの定量評価が可能である。
...
| P9-2 | Meridian プロトコルを通じてシミュレータ（Genesis・Isaac Sim・ROS・MuJoCo のいずれか）と接続し、シミュレーション生成の歩容を実機で再生できること | 実機動作確認 |
| P9-3 | シミュレーション歩容と実機歩容の差異（関節角度差・ZMP 差）を定量的に比較できること | P8 のスクリプトを用いた比較グラフ |
...
フェーズ 9 までの基盤を活かして、より高度な歩行制御と応用研究テーマに取り組む。各テーマは独立しており、関心と優先度に応じて選択・実施する。
...
| S5 | **腕は固定角度** — 肩ロールが `shoulder_roll_angle`（デフォルト 10°、`walkparam-s1.json` で調整可能）の固定値で、歩行位相と連動していない | 脚の振り出しで発生するヨー軸の角運動量が補償されず、上体がわずかに回旋する | 要求 P5-A の実装（位相連動の腕振りへ拡張） |

### 9.4 次に取り組むべきステップ（優先度順）

以下は、現在の達成状況を踏まえて推奨する作業の優先順位である。

#### 優先度 1: ジャイロミキシングの有効化と調整（P5 への着手）

1. **ジャイロミキシングの実機テスト**: `mix_enable=true` に設定し、`mix_gyro_g` のゲインを調整する。まずはロール軸のみのフィードバックで姿勢安定化効果を確認する
2. **ミキシング行列の検証**: 現在のミキシング行列（`MV_MIX_L`, `MV_MIX_R`）が意図通りの関節に作用しているか、足首ロール（index 10）と足首ピッチ（index 9）への対応を確認する

#### 優先度 2: 腕振りの実装（P5-A）

3. **歩行位相連動の腕振り**: 肩ロール角を固定値から `shoulder_roll_angle × sin(phase + π)` に変更し、脚と逆位相で腕を振る制御を追加する。これにより角運動量補償の効果を実験で確認できる

#### 優先度 3: 定量的な歩行評価基盤の整備（P5-B, C）

4. **ZMP のオフライン推定（P5-C）**: `buf_output` に記録された関節角度列から、簡易的なリンクモデル（各リンクの質量は等分配と仮定）で ZMP 軌道を推定する Python スクリプトを作成する
5. **遊脚軌道の比較実験（P5-B）**: 正弦波プロファイル（現行）とサイクロイド軌道で着地衝撃（IMU のピーク加速度）を比較する

#### 優先度 4: フィジカルAI パイプラインの構築（P5-G, H）

6. **模倣学習データ収集パイプライン（P5-G）**: MCP 経由で収集した指令値・実機応答値・IMU センサ値を Genesis AI 等の模倣学習入力フォーマットに整形するスクリプトを作成する
7. **Sim-to-Real 接続（P5-H）**: Meridian プロトコルを通じたシミュレータ接続を行い、シミュレーション歩容と実機歩容の差異を定量的に比較する
...
```
  達成済み              着手中            未着手
┌─────────────┐   ┌──────────┐   ┌──────────────────┐
│ P0 環境      │   │ P5-1 ジャイロ│  │ P5-A 腕振り       │
│ P1 IK        │   │  (基盤のみ) │  │ P5-B 遊脚軌道改善  │
│ P2 1歩       │   │            │  │ P5-C ZMP解析      │
│ P3 周期      │   │            │  │ P5-D 方向転換      │
│ P4 状態遷移  │   │            │  │ P5-E 高度安定化    │
│  (B1,B2,B3  │   │            │  │ P5-F CPG          │
...
```
   ...
   │                          │
[左肩ピッチ]            [右肩ピッチ]
[左肩ロール]  ← 左腕4軸  [右肩ロール]  ← 右腕4軸
[左肘ヨー  ]            [右肘ヨー  ]
[左肘ピッチ]            [右肘ピッチ]
   │                          │
[左股ヨー  ]            [右股ヨー  ]
[左股ロール]  ← 左脚6軸  [右股ロール]  ← 右脚6軸
[左股ピッチ]            [右股ピッチ]
[左膝ピッチ]            [右膝ピッチ]
[左足首ピッチ]          [右足首ピッチ]
[左足首ロール]          [右足首ロール]
...
| 頭部 | 1 | 頭ヨー |
| 腰部 | 1 | 腰ヨー |
| 左腕 | 4 | 左肩ピッチ / 左肩ロール / 左肘ヨー / 左肘ピッチ |
| 右腕 | 4 | 右肩ピッチ / 右肩ロール / 右肘ヨー / 右肘ピッチ |
| 左脚 | 6 | 左股ヨー / 左股ロール / 左股ピッチ / 左膝ピッチ / 左足首ピッチ / 左足首ロール |
| 右脚 | 6 | 右股ヨー / 右股ロール / 右股ピッチ / 右膝ピッチ / 右足首ピッチ / 右足首ロール |
| **合計** | **22** | |

> **歩行制御における使用軸**
> 本仕様書の歩行アルゴリズムが主に制御するのは**脚部 12 軸**である。
> 腕部 8 軸（肩ロール）は歩行中の姿勢保持・腕振り制御（P5-A）に利用する。
