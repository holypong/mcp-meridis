...
以下は、現行の MCP サーバーが提供するツールを用いて P5 の要求を段階的に検証・調整するためのプロンプト例である。AI エージェント（Claude 等）にそのまま渡して使用する。

#### Step 0：現状把握

```
現在の歩行パラメータとロボットの状態を確認してください。
get_params_text でパラメータ一覧を、robot_status で現在の状態を取得してください。
```

#### Step 1：基準歩容の記録

```
以下の手順で基準歩容データを記録してください。
1. get_params_text で現在のパラメータを確認し、記録してください
2. robot_walk で 5 秒間歩行を実行してください
3. 歩行完了後、filesave_buf_output と filesave_buf_input で CSV に保存してください
4. get_buf_output で最新 10 フレームの指令値を表示し、正常にデータが取れているか確認してください
これが以降の比較実験のベースラインになります。
```

#### Step 2：単一パラメータの感度分析（横スイング量）

```
hip_swing パラメータの感度分析を行います。以下の手順を繰り返してください。

1. set_params_text で hip_swing を 0.012 に変更してください
2. robot_walk で 5 秒間歩行し、終了後に filesave_buf_output で保存してください
3. 次に hip_swing を 0.015, 0.018（デフォルト）, 0.021, 0.024 の順に変更し、
   それぞれ同じ手順で歩行・保存してください
4. 各条件の get_buf_input から IMU データ（ロール軸・ピッチ軸）を読み取り、
   姿勢の揺れが最も小さい条件を報告してください

注意：ロボットが転倒した場合は robot_stop で停止し、その値を限界値として記録してください。
```

#### Step 3：単一パラメータの感度分析（遊脚高さ）

```
foot_lift パラメータの感度分析を行います。
hip_swing は Step 2 で最良だった値に固定したまま、
foot_lift を 0.008, 0.011, 0.014（デフォルト）, 0.017, 0.020 の 5 段階で変化させて、
Step 2 と同じ手順で歩行・保存・IMU 比較を行ってください。
足が床を擦る場合は、その値が下限であることを記録してください。
```

#### Step 4：単一パラメータの感度分析（歩行周期）

```
cycle_duration パラメータの感度分析を行います。
hip_swing と foot_lift は前ステップで得た最良値に固定したまま、
cycle_duration を 0.8, 1.0, 1.2（デフォルト）, 1.4, 1.6 の 5 段階で変化させて、
各条件の歩行データを記録してください。
周期が短いほど歩行速度は上がりますが、転倒リスクも高まります。安定歩行できる最短周期を報告してください。
```

#### Step 5：最良パラメータの確定

```
Step 2〜4 の結果を踏まえて、以下をまとめてください。
1. 各パラメータの推奨値と、その根拠（IMU の揺れ、転倒限界との余裕）
2. 推奨値を set_params_text で設定し、robot_walk で 8 秒間の確認歩行を実行
3. filesave_buf_output と filesave_buf_input で最終データを保存
4. get_buf_input で IMU データを確認し、基準歩容（Step 1）との比較所見を報告
```
...
| 状態 | フェーズ |
|---|---|
| ✅ 達成済み | P0 環境構築 / P1 IK / P2 1歩 / P3 周期歩行 / P4 状態遷移 |
| 🔧 基盤のみ | P5 MCP データ収集 / P7 ジャイロミキシング / P9 フィジカルAI |
| ⬜ 未着手 | P6 遊脚軌道・腕振り / P8 ZMP 解析 / P10 発展研究 |
...
| キー名 | 書き込み元 | 読み取り先 | データ内容 |
|---|---|---|---|
| `meridis_mcp_pub` | mcp-meridis | 実機 / merimujoco | 関節角度指令値・サーボコマンド |
| `meridis_sim_pub` | merimujoco | mcp-meridis 等 | シミュレーション状態（関節角度・センサ値） |
| `meridis_calc_pub` | 計算モジュール | 実機 / merimujoco | モーション計算結果 |
| `meridis_mgr_pub` | 実機（マイコンボード） | mcp-meridis 等 | ロボット応答データ（IMU・モータ実測値） |
| `meridis_console_pub` | コンソール | 各モジュール | 手動操作の入力値 |
