...
class RedisPlotter:
    def __init__(self, receiver, fig_width=10, fig_height=5, enable_log=False, display_mode='joint'):
        ...
        if self.display_mode == 'zmp':
            self._setup_zmp_display()
        elif self.display_mode == 'foot':
    ...
    def _update_zmp_plot(self, data) -> list:
        ...
        # 時系列バッファに追記
        self._zmp_t_buf.append(t)
        self._zmp_x_buf.append(result.zmp_x)
        ...
        if poly is not None and len(poly) >= 3:
            ...
            color = 'cyan' if result.is_stable else 'red'
...
def main():
    ...
    parser.add_argument('--log', type=str, default='off', choices=['on', 'off'],
                        help='Redisデータのログ出力')
    parser.add_argument('--display', type=str, default='joint', choices=['joint', 'foot', 'zmp'],
                        help='表示モード: joint=関節角度, foot=足先位置, zmp=ZMP評価（eval_zmp.py 必須）')
    ...
    print(f"ZMP evaluator (eval_zmp.py): {'available' if _EVAL_ZMP_AVAILABLE else 'NOT FOUND – zmp mode unavailable'}")
